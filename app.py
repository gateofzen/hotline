...
def draw_maru(draw, center, r=15):
    x, y = center
...
def render_hotline(header, cases):
    ...
    # 年月日・曜日
    d.text((305, 118), str(dt.year), font=f32, fill="black")
    d.text((432, 118), str(dt.month), font=f32, fill="black")
    d.text((508, 118), str(dt.day), font=f32, fill="black")
    d.text((590, 118), wd, font=f32, fill="black")

    # 日勤/夜勤 ◯（解析: X=686-701=日勤, X=763-793=夜勤）
    if header["shift"] == "日勤":
        draw_maru(d, (693, 138), r=20)
    else:
        draw_maru(d, (778, 138), r=20)

    # リーダー医師名
    d.text((1010, 118), header["leader"], font=f36, fill="black")
    ...
    for i, case in enumerate(cases):
        ...
        # --- No.+時刻行 (yt〜yt+32, テキストY = yt+5) ---
        yn = yt + 5

        # 時刻（テンプレートに "時刻 :" 印刷済み → 直後に時刻文字列）
        if case.get("time"):
            d.text((260, yn), case["time"], font=f36, fill="black")

        # 依頼回数 初回◯ (X=615-644) または 数字
        if case.get("req_count") == "初回":
            draw_maru(d, (620, yn + 16), r=18)
        else:
            num = (case.get("req_count") or "").replace("回目","").replace("回","")
            if num:
                d.text((718, yn), num, font=f36, fill="black")

        # 依頼先（X=1156-1174 付近）
        if case.get("team"):
            d.text((920, yn), case["team"], font=f36, fill="black")

        # --- 症例行 (yt+32〜yt+65, テキストY = yt+33) ---
        yc = yt + 33

        # 年齢（"年齢(" の直後 X≈215, "才)" の前）
        if case.get("age"):
            d.text((218, yc), str(case["age"]), font=f36, fill="black")

        # 性別 M(X≈490) / F(X≈530)  ← 解析済み
        if case.get("gender") == "M":
            draw_maru(d, (490, yc + 16), r=18)
        elif case.get("gender") == "F":
            draw_maru(d, (533, yc + 16), r=18)
        ...
        for li, ln in enumerate(lines[:2]):
            d.text((160, yg + li*34), ln, font=f28, fill="black")

        # --- 転帰行 (yt+118〜yt+219) テキストY ≈ yt+125 ---
        # 解析: Y=293=yt+125 に "転帰：□搬入 □お断り □2次 □患者都合" あり
        yn2 = yt + 125
        ...
        if outcome == "お断り":
            ...
            if reason == "1_満床":
                draw_maru(d, (130, yr1), r=12)
                sub_map = {
                    "満床":              (263, yr1),
                    "満床に準ずる状態":   (405, yr1),
                    "ICU個室(感染等)満床": (600, yr1),
                    "熱傷患者受入不能":   (815, yr1),
                }
...
for i, c in enumerate(st.session_state.hl_cases):
    ...
    with col_a:
        st.write(f"**症例{i+1}**　{time_str}　{team_str}救急隊　→ {outcome_str}")
    with col_b:
...
if n < 6:
    ...
    with cc2:
        team = st.selectbox("依頼先救急隊", RESCUE_TEAMS, key="inp_team")

    cc3, cc4, cc5 = st.columns(3)
    with cc3:
        req_count = st.selectbox("依頼回数", ["初回","2回目","3回目","4回目以上"], key="inp_req")
    with cc4:
        age = st.number_input("年齢（才）", min_value=0, max_value=120, value=0, step=1, key="inp_age")
    with cc5:
        gender = st.radio("性別", ["M","F","未記載"], horizontal=True, key="inp_gender")
    ...
    if st.button("✅ この症例を登録", type="primary", use_container_width=True):
        st.session_state.hl_cases.append({
            ...
            "outcome": outcome,
            "reason": reason,
            "reason1_sub": reason1_sub,
            "reason2_sub": reason2_sub,
